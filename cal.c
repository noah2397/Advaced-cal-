...
void main(void) {
	...
	while(1){
		...
		else if(strcmp(input,"3") == 0){//저번달 멀리뛰기
			clear();
			move(0,0);
			printw(">>%s",input);
			int num;
		...
		else if(strcmp(input,"4") == 0){//다음달 멀리뛰기
			clear();
			move(0,0);
			printw(">>%s",input);
			int num;
		...
		else if(strcmp(input,"7") == 0)//아날로그 시계 & 디지털 시계 구현
			print_analogclock();
...
void delete_schedule(struct tm t, int record){ //레코드 지우기
...
void print(struct tm t, int unit){//stdout으로 달력 출력
...
int return_unit(int month, int year){//월과 년을 참고하여 해당 달에 몇일이 있는지 파악하는 함수
	int unit;
	if(month < 1){
		month = 12;
		year -= 1;
...
int is_Leaf(int year){//윤년 계산함수
	int check;
	if((year % 4 == 0 && year % 100 != 0) || year % 400 == 0) check = 1;
	else check = 0;
	return check;
}
void schedule(struct tm t){//스케줄 등록 함수(파일에다가 쓴다)
...
void search(int y, int m){//화면을 불러올때마다 레코드에서 해당 년월에 상응하는 레코드들을 찾고 출력하는 함수
	int index =30; 
	char content[100];
	char year[10];
	char month[10];
...
void print2(struct tm t, int unit){//달력을 출력하는 함수
	start_color();
	init_color(0,0,0,0);
	init_pair(1, 1, 0);
	init_pair(2, 4, 0);
...
void colon(int d)// "*"콜론을 출력하는 함수 : 2019 * 10 * 9 일때의 *을 콜론이라고 가정
{
    move(7, d*10);
    for(int y=0;y<5;y++)
    {
...
void DrawNum(int dn,int n)//숫자를 3차원 배열을 사용해 출력
{
    int y,x;
...
void input_date(int y, int m, int d, char* input, int unit, struct tm *t)//6번 기능 : 날짜 계산기에 사용되는 함수
{
	clear();
	move(0,0);
	printw(">>%s",input);
...
float cosine(int sec)//코사인 함수, 값 반환
{
        float temp;         
